...
- **`rule_regions`**: Region-scoped rule overrides (requires `featureFlags.ruleRegions`; default off). An array of regions, each an axis-aligned box plus a layer set carrying a clearance override and optionally a trace half-width override, e.g.
  ...
  Regions that share a `clearance_um` value share one internal clearance class, so the cost of the feature scales with the number of DISTINCT clearance values, not the number of regions — the usual configuration (many exception zones at one design-rule clearance) costs the same as a single region. Each distinct clearance value does cost a full-board search tree at route time, so prefer reusing one clearance value across zones over giving each zone its own slightly different number.
